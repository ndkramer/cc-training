...
window.SLIDES = [
  ...
  { c: 'Slide_ColdOpen', section: 'pre', kind: 'content', mins: 1,
    notes: "[Idle shows `$ claude` + cursor. Press Enter to arm — animation runs ~6s. Don't speak until the title lands.]\n\nThat was Claude Code. A test was failing in order-service — code that's been around a while, let's say — and it found the bug and fixed it. Six seconds of wall-clock. Everything in the next three hours is some version of what you just watched. We're just going to get progressively more ambitious about what 'the mess' is." },

  { c: 'Slide_LinaAgenda', section: 'pre', kind: 'content', mins: 1,
    notes: "Five sections. Introduction — what Claude Code is and where it fits. How to work with it — modes, commands, the daily-driver shortcuts. Context management — the longest section, because output quality is context quality. Common workflows — spec-driven and test-driven patterns. And an appendix with the deeper-cut features.\n\nIf you only remember one section, make it section three." },

  // ── 101 — FOUNDATIONS ────────────────────────────────────────────

  { c: 'Slide_Section101', section: '101', kind: 'section', mins: 0.5,
    notes: "Foundations. What it is, how to get it running, then straight into a live demo." },

  { c: 'Slide_WhatIsCC', section: '101', kind: 'content', mins: 1,
    notes: "Claude Code is an agent in your terminal. Not a sidebar, not autocomplete, not a browser tab. It lives in your repo. You describe what you want, it reads your files, runs your tests, makes edits — and every tool call shows up in the terminal. Every edit is a diff you see before it lands. Agent, terminal, repo, visible. That's the whole model." },

  { c: 'Slide_WhatIsLoop', section: '101', kind: 'content', mins: 1.5,
    notes: "[2 clicks. Click 1 — loop animates. Click 2 — 'who it's for' card reveals.]\n\nHere's what 'agent' actually means. Four steps, looping.\n\n[Click 1] Read — it looks at your code. Decide — it figures out what to do next. Act — it runs a command or makes an edit. Observe — it checks what happened. And then it loops. You describe the goal, it runs that loop until it gets there. That's the whole mechanism. Not magic — just this, repeated.\n\n[Click 2] Who's it for: anyone who writes code. Backend, frontend, infra, data. If you're in a terminal and a repo, this works the same way. The loop doesn't care what language you're in." },

  { c: 'Slide_Install', section: '101', kind: 'content', mins: 1,
    notes: "Getting it installed. You're on c4e, so this is the easy path: one install, then `claude` in your terminal, and it walks you through auth. No API keys to manage, no env vars. Your admin already provisioned the seats.\n\nOne caveat — first run downloads the binary, takes about thirty seconds. After that it's instant." },

  { c: 'Slide_Models', section: '101', kind: 'content', mins: 1,
    notes: "Three models. Opus for the hard problems — big refactors, gnarly debugging, when you need it to really think. Sonnet for daily driving — fast enough, smart enough, this is your default. Haiku for speed — formatting, boilerplate, anything where you know exactly what you want and you just need it typed.\n\nYou switch with `/model` inside a session, mid-conversation. Don't memorize version numbers — they change. The slash command always lists what's current." },
  ...
  { c: 'Slide_WorkflowTransform', section: '101', kind: 'content', mins: 1.5,
    notes: "The point isn't the number — it's where the time goes. Before: most of your hours are in implementation and debugging. After: most of your hours are in spec, review, architecture. Your value moves up the stack — from code production to design judgment. Claude does the typing; you do the thinking." },

  { c: 'Slide_SDLC', section: '101', kind: 'content', mins: 3,
    notes: "[6 substages. Intro lands automatically — then 5 arrow-right clicks walk Discover → Design → Build → Deploy → Support. Each stage runs ~5s of animation; let it breathe before clicking through.]\n\nHere's the thing that surprises people: Claude Code isn't just for writing code. It helps across the entire software development lifecycle.\n\n[Click → Discover] Exploring an unfamiliar codebase. Claude reads README, CLAUDE.md, walks the order module. Same thing you'd do — but in seconds.\n\n[Click → Design] Drafting an RFC. Problem statement, proposed solution, API changes. The doc streams in.\n\n[Click → Build] The actual edit. Diff view — config.baseUrl replaces the hardcoded URL.\n\n[Click → Deploy] PR opened, CI runs, checks pass, merge button lights up. The terminal narrates while GitHub renders.\n\n[Click → Support] Sentry alert. 847 events. Stack trace points at pricing.js:12 — the same line we just fixed. Claude reads the alert, finds the root cause, links it to the PR.\n\nFive stages. Same agent. That's the loop — discovery through hotfix." },

  { c: 'Slide_NoOneWay', section: '101', kind: 'content', mins: 1,
    notes: "[Click once — kicker. Click again — tweet lands.]\n\nThis is Boris. He created Claude Code. And what he wants you to know is the underlined bit: there is no one right way to use this.\n\nEverything I show you today — CLAUDE.md, plan mode, skills — these are patterns, not rules. The way I set it up for order-service is not how you'll set it up for your stack. That's fine. That's the point. Steal what's useful, ignore what isn't." },

  { c: 'Slide_Boris', section: '101', kind: 'content', mins: 2,
    notes: "[3 clicks.]\n\n[Click 1 — tweet.] That's the tweet again. But the thing is, Boris doesn't just say 'experiment' and leave it there. Here's what his day actually looks like.\n\n[Click 2 — phone with 5 sessions.] Five Claude sessions running at once. One's reviewing a PR. One's in a feature branch. One's just answering questions. He context-switches between them the way you'd context-switch between browser tabs. And on c4e — these sessions aren't all on his laptop. Some are in the browser. One might be on his phone.\n\n[Click 3 — counter lands.] Twenty PRs a day. That's not a vanity number — that's what happens when five sessions are each closing small loops in parallel.\n\nYou don't have to work this way. But this is what the ceiling looks like." },
  ...
  { c: 'Slide_Rookie', section: '101', kind: 'content', mins: 2,
    notes: "[6 clicks — one card per click.]\n\nEven experienced users make these. Each card is symptom → why → fix.\n\nWatching it work, approving every edit? Shift-Tab to auto-accept. Long task holding the prompt? Ctrl-B backgrounds it. Wrong file, you nuke the session? Esc ONCE interrupts — steer with one sentence, keep your context. Typing paths from memory? @-prefix tab-completes, fuzzy. Day three same session, it's confused? /clear between tasks. Gnarly bug, it's thrashing? Plan mode — read-only, propose first.\n\nThese aren't advanced. They're week-one habits." },
  ...
  { c: 'Slide_Section201', section: '201', kind: 'section', mins: 0.5,
    notes: "That was cold. Out of the box, on code it's never seen. Here's where it gets actually good — when you teach it the repo." },
  ...
  { c: 'Slide_ClaudeMd', section: '201', kind: 'content', mins: 2,
    notes: "The first thing you teach it with is CLAUDE.md. Markdown file, root of the repo, Claude reads it automatically every session.\n\nThink of it as the briefing you'd give a very smart contractor. How do I run the tests. What's the build command. What should I never touch. For a legacy codebase — what's the known tech debt. Which modules are fragile. Where are the dragons.\n\nKeep it short. Under 200 lines. You're not writing documentation, you're writing a briefing. The stuff a new engineer would spend a week asking around about — that goes here. The stuff they can grep for — leave it out." },

  { c: 'Slide_ClaudeMdNesting', section: '201', kind: 'content', mins: 2,
    notes: "CLAUDE.md nests. Three levels.\n\nYour personal one, home directory — follows you across every repo. The repo root one — build conventions, how the codebase is laid out. And per-subdirectory — each module has its own.\n\nThey merge like CSS. The subdirectory CLAUDE.md doesn't restate the build stuff — that's inherited from root. It adds what's specific to that corner: which functions are pure, which ones touch the database, which ones nobody has dared refactor since 2019.\n\nSo when you launch Claude inside a subdirectory, it's reading all three layers. It knows the org conventions AND it knows the corner you're in. You don't repeat yourself, and Claude doesn't have to guess." },
