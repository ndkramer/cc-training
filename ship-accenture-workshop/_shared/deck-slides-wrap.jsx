...
function Slide_Title({ openConfig }) {
  return (
    <Slide bg={DARK} color={BONE} label="Title" center>
      <img src="../_shared/assets/white-spark.svg" alt="" style={{ width: 44, height: 44, marginBottom: 28 }} />
      <h1 style={{
  ...
  return (
    <Slide bg={BONE} label="Recap">
      ...
      <div style={{
        ...
        <strong style={{ color: CLAY }}>Keep the repo.</strong> The flag-service
    ...
    <Slide bg={BONE} label="Resources">
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{
          ...
          borderLeft: `3px solid ${CLAY}`, marginBottom: 24, maxWidth: 520,
        }}>
          <div style={{ fontFamily: MONO, fontSize: 15, color: DARK, marginBottom: 4 }}>
            <span style={{ color: GRAY }}>{'> '}</span>/powerup
          </div>
          <div style={{ fontSize: 12, color: GRAY }}>
            Interactive lessons, in your terminal. The fastest way to keep going tomorrow morning.
          </div>
        </div>
        <PlatformSwitch>
          <div data-platform="c4e">
            <LinkRow href="anthropic.com/learn/claude-for-work" label="Claude for Work" />
            <LinkRow href="code.claude.com/docs" label="Full docs" />
            <LinkRow href="code.claude.com/docs/en/claude-code-on-the-web" label="Claude Code Web" />
            <LinkRow href="code.claude.com/docs/en/skills" label="Writing skills" />
            <LinkRow href="code.claude.com/docs/en/mcp" label="MCP connectors" />
          </div>
          <div data-platform="bedrock">
            <LinkRow href="code.claude.com/docs" label="Full docs" />
            <LinkRow href="code.claude.com/docs/en/amazon-bedrock" label="Bedrock setup" />
            <LinkRow href="code.claude.com/docs/en/mcp" label="MCP connectors" />
            <LinkRow href="code.claude.com/docs/en/skills" label="Writing skills" />
          </div>
          <div data-platform="vertex">
            <LinkRow href="code.claude.com/docs" label="Full docs" />
            <LinkRow href="code.claude.com/docs/en/google-vertex-ai" label="Vertex setup" />
            <LinkRow href="code.claude.com/docs/en/mcp" label="MCP connectors" />
            <LinkRow href="code.claude.com/docs/en/skills" label="Writing skills" />
          </div>
          <div data-platform="azure">
            <LinkRow href="code.claude.com/docs" label="Full docs" />
            <LinkRow href="code.claude.com/docs/en/microsoft-foundry" label="Azure setup" />
            <LinkRow href="code.claude.com/docs/en/skills" label="Writing skills" />
          </div>
          <div data-platform="default">
            <LinkRow href="code.claude.com/docs" label="Full docs" />
            <LinkRow href="platform.claude.com" label="API console & keys" />
            <LinkRow href="code.claude.com/docs/en/skills" label="Writing skills" />
            <LinkRow href="code.claude.com/docs/en/mcp" label="MCP servers" />
...
// ── Thanks ────────────────────────────────────────────────────────
// AE contact via ?ae= URL param, e.g. deck.html?ae=jane.doe@anthropic.com
function Slide_Thanks() {
  var raw = new URLSearchParams(location.search).get("ae") || "your-ae@anthropic.com";
  var ae = raw.split("?")[0];
  return (
    <Slide bg={DARK} color={BONE} label="Thanks" center>
      <img src="../_shared/assets/white-spark.svg" alt="" style={{ width: 40, height: 40, marginBottom: 24 }} />
      <h1 style={{ fontFamily: SERIF, fontSize: 48, fontWeight: 380, marginBottom: 32 }}>Thank you</h1>
      <div style={{ fontSize: 13, color: 'rgba(250,249,245,0.4)', fontFamily: MONO }}>
        {ae}
      </div>
    </Slide>
  );
...
// ── /powerup ──────────────────────────────────────────────────────
// Issue #19. Self-serve next step — interactive lessons in-product.
function Slide_Powerup() {
  return (
    <Slide bg={DARK} color={BONE} label="Powerup" center>
      <div style={{ ...KICKER, color: 'rgba(250,249,245,0.5)' }}>Keep going</div>
      <code style={{
        fontFamily: MONO, fontSize: 56, fontWeight: 400,
        display: 'block', margin: '24px 0 16px',
      }}>
        <span style={{ color: 'rgba(250,249,245,0.35)' }}>{'> '}</span>
        <span style={{ color: CLAY }}>/powerup</span>
      </code>
      <p style={{ fontSize: 16, color: 'rgba(250,249,245,0.7)', maxWidth: 460, lineHeight: 1.6 }}>
        Interactive lessons inside the terminal. Animated demos of every feature
        we covered today — and the ones we didn't. Run it tomorrow morning.
      </p>
      <p style={{ fontSize: 13, color: 'rgba(250,249,245,0.45)', marginTop: 28, maxWidth: 460 }}>
        Rolling out to your team?{' '}
        <code style={{ fontFamily: MONO, color: CLAY }}>/team-onboarding</code>{' '}
        turns your own usage into their ramp-up guide.
      </p>
    </Slide>
  );
}
...
Object.assign(window, {
  ...
  Slide_Recap, Slide_Resources, Slide_Powerup, Slide_QA, Slide_Thanks,
});
